...
namespace APILibros.Controllers
{
    ...
    public class AutoresController : ControllerBase
    {
        ...
        {
            var autores = await _context.Autor.AsNoTracking().ToListAsync();
            return Ok(autores);
        }
        ...
        {
            var autor = await _context.Autor.AsNoTracking().FirstOrDefaultAsync(a => a.IdAutor == id);

            if (autor == null)
            {
                return NotFound(new {msg = $"No se encontró el autor con Id {id}."});
            }
        ...
        {
            var autorExiste = await _context.Autor.AnyAsync(a => a.IdAutor == id);
            ...
            var libros = await _context.Libros.AsNoTracking().Where(l => l.IdAutor == id).ToListAsync();

            if (libros == null || libros.Count == 0)
            {
                return Ok(new{msg = "El autor no tiene libros registrados.", libros = new List<LibrosModel>()});
            }
        ...
        public async Task<IActionResult> AgregarAutor([FromBody] AutorModel autor)
        {
            if (autor == null)
            {
                return BadRequest(new{msg = "Debe enviar los datos del autor."});
            }
        ...
        public async Task<IActionResult> ActualizarAutor(int id, [FromBody] AutorModel autorActualizado)
        {
            ...
            {
                return BadRequest(new{msg = "Debe enviar los datos actualizados del autor."});
            }
